		...
		handler: function (ev) {
			...
			switch (ev.type) {
				...
				case 'touchmove':
					if(touchStart === true) {
						...
						if (posX >= 0) {
							...
							if (current_pane === 0) {
								$('.footer').css('opacity', 1 - opa);
								$('.screen').css('opacity', opa);
							}
						} else if (posX < 0) {
				...
				case 'touchend':
					...
					if (opa >= 1) {
						if (posX > 0) {
							panes.eq(current_pane).animate({"transform": "translate(" + (pane_width + 150) + "px," + (posY - pane_width + 100) + "px) rotate(60deg)"}, $that.settings.animationSpeed, function () {
								if($that.settings.onLike) {
									$that.settings.onLike(panes.eq(current_pane));
								}
								$that.next();
